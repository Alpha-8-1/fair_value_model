...
def main():
    ...
    # Correctly pass only 8 arguments (skip sp500_price)
    base_fair_value, adjusted_fair_value, adjustments = calculate_fair_value(
        data[0],  # forward_eps
        data[1],  # equity_risk_premium
        data[3],  # yield_10y
        data[4],  # cpi_yoy
        data[5],  # pmi
        data[6],  # vix_value
        data[7],  # credit_spread_proxy
        data[8]   # sentiment_score
    )
    ...
    print(f"10Y Treasury Yield (Raw pulled): {yield_10y * 100:.2f}%")
